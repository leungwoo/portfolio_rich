...
function Navbar() {
  ...
  return (
    <nav className="appBar flex flex-row justify-between items-center px-4 py-3 w-full backdrop-blur-sm backdrop-filter z-50 border-b-2 border-navbarborder md:border-transparent fixed dark:text-white">
      <a href="#HOME" className="logo flex justify-start items-center ">
        <img
          src={avatarrest}
          alt="logo"
          className="flex-1 object-cover mb-2 max-w-[60px] max-h-[60px] shadow-primary shadow-sm dark:shadow-white rounded-full "
        />
      </a>
      <ul className="links hidden md:flex flex-1 justify-end items-center list-none text-base">
        {[
          ...
          <li
            key={item}
            className="flex flex-col mx-4 cursor-pointer justify-center items-center "
          >
            <motion.a
      ...
      <div className="cursor-pointer hover:text-primary hover:transition-all duration-500 ease-in-out ">
        {darkMode ? (
          <BsSunFill onClick={() => setDarkMode(!darkMode)} size={20} />
        ) : (
          <BsFillMoonStarsFill
